...
async function resolveCoordinates(input, output) {
  ...
  try {
    ...
    // ARTEK route-sources signature: (routerPath, srcRoot) — resolves both
    // `@pages` and `@shared` aliases against src/.
    const map = buildRouteMap(
      path.join(wwwRoot, 'src/router/index.tsx'),
      path.join(wwwRoot, 'src'),
    );
...
export async function generate(input, output, opts) {
  ...
  if (opts.header) {
    ...
    // Notice template follows the OUTPUT locale (en.txt → template.en.md),
    // so every generated file opens in its own language.
    const outLocale = path.basename(output, '.txt');
    out = fillTemplate(loadTemplate(outLocale), coords, body).trimEnd() + '\n';
  }
