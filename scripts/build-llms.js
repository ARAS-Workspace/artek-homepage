 ...
 *
 * Usage:
 *     node scripts/build-llms.js   (requires dist/ — run the build first)
 */
...
// so it cannot mark a section break unambiguously. A long rule is still a
// markdown HR, occurs nowhere in the content, and splits cleanly on /^-{4,}$/.
...
function fullHeader(locale, pageCount) {
  ...
  return [
    ...
    '>',
    `> **EN:** All ${pageCount} pages of the ARTEK website, concatenated from`,
    ...
    '> (diagrams, charts, surveys, data tables) are replaced by their source',
    '> data or a short directive — read those as data pointers, not missing content.',
    '>',
    `> **TR:** ARTEK web sitesinin ${pageCount} sayfasının tamamı, ilgili sayfanın`,
    ...
    '> (diyagram, grafik, anket, veri tablosu) kaynak verisiyle veya kısa bir',
...
for (const [route, dir] of [...routeMap].sort((a, b) => a[0].localeCompare(b[0]))) {
  ...
  // route '/' → dist/llms/ ; '/company' → dist/company/llms/
